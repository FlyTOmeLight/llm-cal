...
def analyze(
    *,
    ...
    if cluster_tps <= 0 or target_tokens_per_sec <= 0 or degradation <= 0:
        ...
        l = max(0, math.floor(cluster_tps / target_tokens_per_sec / degradation))
    ...
    if k == 0 and l == 0:
        ...
        reason_en = (
            f"K ({k}) ≤ L ({l}) → memory-capacity bound. "
            "KV cache exhausts GPU headroom before throughput SLA does."
        )
        reason_zh = f"K ({k}) ≤ L ({l}) → 显存容量瓶颈。先达到 KV cache 容量上限，才到吞吐目标。"
    else:
        max_n = l
        ...
        reason_en = (
            f"L ({l}) < K ({k}) → memory-bandwidth / compute bound. "
            "Cluster can't sustain target tok/s per user at this concurrency."
        )
        reason_zh = f"L ({l}) < K ({k}) → 带宽/算力瓶颈。集群在此并发下无法维持目标 tok/s。"
    ...
    return ConcurrencyAnalysis(
        ...
        l_bound=AnnotatedValue(l, l_label, source=l_source),
